...
// InstallRequestData is the request data for installing new WeManageIt app
...
func (rs authResource) Routes(wemanageitHandler TaskcafeHandler) chi.Router {
	r := chi.NewRouter()
	r.Post("/login", wemanageitHandler.LoginHandler)
	r.Post("/logout", wemanageitHandler.LogoutHandler)
	r.Post("/validate", wemanageitHandler.ValidateAuthTokenHandler)
	return r
}
