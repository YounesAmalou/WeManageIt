...
import (
	...
	"WeManageIt/internal/db"
	"WeManageIt/internal/frontend"
	"WeManageIt/internal/graph"
	"WeManageIt/internal/jobs"
	"WeManageIt/internal/logger"
)
...
func NewRouter(dbConnection *sqlx.DB, redisClient *redis.Client, jobServer *machinery.Server, appConfig config.AppConfig) (chi.Router, error) {
	...
	wemanageitHandler := TaskcafeHandler{*repository, appConfig}

	var imgServer = http.FileServer(http.Dir("./uploads/"))
	r.Group(func(mux chi.Router) {
		mux.Mount("/auth", authResource{}.Routes(wemanageitHandler))
		mux.Handle("/__graphql", graph.NewPlaygroundHandler("/graphql"))
		mux.Mount("/uploads/", http.StripPrefix("/uploads/", imgServer))
		mux.Post("/auth/confirm", wemanageitHandler.ConfirmUser)
		mux.Post("/auth/register", wemanageitHandler.RegisterUser)
		mux.Get("/settings", wemanageitHandler.PublicSettings)
		mux.Post("/logger", wemanageitHandler.HandleClientLog)
	})
	...
	r.Group(func(mux chi.Router) {
		...
		mux.Post("/users/me/avatar", wemanageitHandler.ProfileImageUpload)
