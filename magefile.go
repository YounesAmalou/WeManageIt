...
func (Backend) Build() error {
	fmt.Println("compiling binary dist/wemanageit")
	return runWith(flagEnv(), "go", "build", "-ldflags", ldflags, "-tags", "prod", "-o", "dist/wemanageit", "cmd/wemanageit/main.go")
}
...
func (Backend) Test() error {
	fmt.Println("running wemanageit backend unit tests")
...
func Release() error {
	...
	version, ok := os.LookupEnv("WEMANAGEIT_RELEASE_VERSION")
	if !ok {
		return errors.New("WEMANAGEIT_RELEASE_VERSION must be set")
	}
	if !semverRegex.MatchString(version) {
		return errors.New("WEMANAGEIT_RELEASE_VERSION must be a valid SemVer")
	}
	...
	err = sh.RunV("docker", "build", ".", "-t", "wemanageit/wemanageit:latest", "-t", "wemanageit/wemanageit:"+version)
	if err != nil {
		return err
	}
	err = sh.RunV("docker", "push", "wemanageit/wemanageit:latest")
	if err != nil {
		return err
	}
	err = sh.RunV("docker", "push", "wemanageit/wemanageit:"+version)
	if err != nil {
		return err
	}
...
// Up starts the docker-compose file using the `latest` wemanageit image
func (Latest) Up() error {
	return sh.RunV("docker-compose", "-p", "wemanageit-latest", "-f", "testing/docker-compose.latest.yml", "up")
}
...
func (Dev) Up() error {
	return sh.RunV("docker-compose", "-p", "wemanageit-dev", "-f", "testing/docker-compose.dev.yml", "up")
}
...
func (Docker) Up() error {
	return sh.RunV("docker-compose", "-p", "wemanageit", "up", "-d")
}

// Migrate runs the migration command for the docker-compose network
func (Docker) Migrate() error {
	return sh.RunV("docker-compose", "-p", "wemanageit", "-f", "docker-compose.yml", "-f", "docker-compose.migrate.yml", "run", "--rm", "migrate")
}
